      ...
      <div className="text-center py-4">
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-4 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-gray-500 line-through">RM {suggestion.currentTotal.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-bold text-gray-900">Meal deal price</span>
            <span className="font-bold text-mcdonalds-red">RM {suggestion.comboPrice.toFixed(2)}</span>
          </div>
          <div className="border-t border-gray-200 pt-2 flex justify-between">
            <span className="font-bold text-green-600">You save</span>
            <span className="font-bold text-green-600">RM {suggestion.savings.toFixed(2)}</span>
          </div>
        </div>

        <div className="space-y-2">
          <button onClick={onConvert} className="w-full btn-primary py-3 text-lg">
            Convert to Meal - Save RM {suggestion.savings.toFixed(2)}
          </button>
