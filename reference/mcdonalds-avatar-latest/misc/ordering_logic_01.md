...
### 1.4 Smart Meal Conversion Acceptance

#### Pattern: Conversion with Meal Details in Response
**User Input**: "I want a Big Mac"
**Avatar**: "Would you like to make your Big Mac a meal? It comes with a side and drink!"
**User Input**: "Yeah make it a meal with large Coke"

**Flow**:
1. Detect meal conversion acceptance
2. Parse meal details from acceptance response:
   - Size: Not specified
   - Drink: "Coke" (detected)
   - Side: Not specified
   - Ice: Not specified
3. Pre-populate detected details in meal state
4. Skip to first missing detail
5. Avatar: "Great! What side would you like? We have fries, corn cup, or garden salad."
6. Continue from first missing component

**Benefits**:
- Reduces number of questions asked
- More natural conversation flow
- Faster ordering experience
- Users can specify details proactively

#### Pattern: Full Details in Acceptance
**User Input**: "I want a Big Mac"
**Avatar**: "Would you like to make your Big Mac a meal?"
**User Input**: "Yes, large with fries and Coke, full ice"

**Flow**:
1. Parse all meal details from response:
   - Size: "large"
   - Side: "fries"
   - Drink: "Coke"
   - Ice: "full"
2. All components detected - meal is complete
3. Add to cart immediately
4. Avatar: "Perfect! I've added a large Big Mac meal with fries and Coke with full ice. Anything else?"

#### Pattern: Partial Details in Acceptance
**User Input**: "I want a McChicken"
**Avatar**: "Would you like to make that a meal?"
**User Input**: "Sure, large with Sprite"

**Flow**:
1. Parse detected details:
   - Size: "large"
   - Drink: "Sprite"
2. Missing: side, ice level
3. Avatar: "Great! What side would you like? Fries, corn cup, or garden salad?"
4. Wait for response: "Fries"
5. Avatar: "Would you like ice with your Sprite? Full ice, less ice, or no ice?"
6. Complete meal customization

#### Implementation Notes

**Parsing Strategy**:
```typescript
interface MealDetailsFromAcceptance {
  size?: 'medium' | 'large';
  side?: string;
  drink?: string;
...
function parseMealDetailsFromAcceptance(userText: string): MealDetailsFromAcceptance {
  ...
  if (side) details.side = side.name;

  // Parse drink
  const drink = findDrinkByName(userText);
  if (drink) details.drink = drink.name;

  // Parse ice level
  if (/\b(no ice|without ice|none)\b/i.test(userText)) {
    details.iceLevel = 'none';
  } else if (/\b(less ice|little ice|light ice)\b/i.test(userText)) {
    details.iceLevel = 'less';
  } else if (/\b(full ice|normal ice|regular ice|with ice)\b/i.test(userText)) {
    details.iceLevel = 'full';
  }
...
**Integration Point**:
- Modify `parseMealConversionResponse()` in `lib/ordering/mealConversion.ts`
- Return both `accepted` status AND parsed `mealDetails`
- Update `convertItemToMeal()` to accept optional pre-populated details
- Modify question flow to skip already-provided details

**Example Responses to Handle**:
```
✅ "Yes" → No details
✅ "Yeah make it a meal" → No details
✅ "Sure, large" → Size only
✅ "Yes with Coke" → Drink only
✅ "Yeah large with fries" → Size + side
✅ "Make it a large meal with fries and Coke" → Size + side + drink
✅ "Yes, large with fries and Coke, no ice" → All details
```

**Priority**: Medium-High
- Enhances user experience significantly
- Reduces friction in ordering process
- Not critical for MVP but high value-add
